...
describe('resource event_subscriptions', () => {
  ...
  test('create: required and optional params', async () => {
    const response = await increase.eventSubscriptions.create({ url: 'https://website.com/webhooks',shared_secret: 'x',selected_event_category: 'account.created' })
  })
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(increase.eventSubscriptions.retrieve('string', { path: '/_stainless_unknown_path' }))
    .rejects
  ...
  test('list: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(increase.eventSubscriptions.list({ path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Increase.NotFoundError)
  })
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(increase.eventSubscriptions.list({ cursor: 'string',limit: 0 }, { path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Increase.NotFoundError)
  })
})