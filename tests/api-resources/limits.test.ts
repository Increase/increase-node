...
describe('resource limits', () => {
  ...
  test('create: required and optional params', async () => {
    const response = await increase.limits.create({ metric: 'volume',interval: 'month',model_id: 'account0',value: 1234 })
  })
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(increase.limits.retrieve('string', { path: '/_stainless_unknown_path' }))
    .rejects
  ...
  test('list: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(increase.limits.list({ path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Increase.NotFoundError)
  })
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(increase.limits.list({ cursor: 'string',limit: 0,model_id: 'x',status: 'x' }, { path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Increase.NotFoundError)
  })
})