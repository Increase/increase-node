...
describe('resource ach_transfers', () => {
  test('create_inbound: only required params', async () => {
    const response = await increase.simulations.achTransfers.createInbound({ account_number_id: 'account_number_v18nkfqm6afpsrvy82b2',amount: 1000 })
  ...
  test.skip('return: request options instead of params are passed correctly', async () => {
    ...
    await expect(increase.simulations.achTransfers.return('string', { path: '/_stainless_unknown_path' }))
    .rejects
  ...
  test.skip('submit: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(increase.simulations.achTransfers.submit('string', { path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Increase.NotFoundError)
  })
})