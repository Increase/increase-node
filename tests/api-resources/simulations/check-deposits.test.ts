...
describe('resource check_deposits', () => {
  ...
  test.skip('reject: request options instead of params are passed correctly', async () => {
    ...
    await expect(increase.simulations.checkDeposits.reject('string', { path: '/_stainless_unknown_path' }))
    .rejects
  ...
  test.skip('submit: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(increase.simulations.checkDeposits.submit('string', { path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Increase.NotFoundError)
  })
})